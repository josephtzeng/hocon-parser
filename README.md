...
A lot of examples can be found in the [tests/resources/configs](https://github.com/josephtzeng/hocon-parser/tree/master/tests/resources/configs) directory. Each subdirectory should contain a file ``test.conf`` and optionally a result file ``result.json``. If a directory name contains the ``.error`` suffix, that means parsing the test file ``test.conf`` will result in an error. By default, the files are parsed in strict mode when testing. If the directory name has a ``.default`` suffix, the config file will be parsed in the default mode.
...
Besides the ``sort`` and ``slice`` transforms, the parse comes with several useful transforms like ``map``, ``reduce``, ``query``, and ``eval``. You can also define your transforms and register them with the ``parser.registerComponent ()`` method.
...
| Protocol    | Source Class     | Purpose                                                       |
|-------------|------------------|---------------------------------------------------------------|
| context:    | ContextSource    | Loads text from Context.text                                  |
| file:       | FileSource       | Loads a file at the given path                                |
| http:       | HttpSource       | Loads a file at the given URL whose scheme is http            |
| https:      | HttpsSource      | Loads a file at the given URL whose scheme is https           |
| modulepath: | ModulepathSource | Loads a file under the module search paths (``module.paths``) |
