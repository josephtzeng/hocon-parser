...
function compileExprForArray (array, expr, aliases)
{
    ...
    try
    {
        let func = Function.apply (null, (aliases || []).concat (props, body)); // eslint-disable-line no-new-func

        func.props  = props;
        func.body   = body;

        func.invoke = function (el)
        {
            var args  = toArray (arguments)
                        .slice (1)
                        .concat (props.map ((p) => el[p]));

            return func.apply (this == func ? el : this, args);
        };
...
function arrayReduce (array, expr, initValue)
{
    ...
    var args  = [(acc, el, i) => func.invoke (el, el, acc, i, array)];
...
function arrayMap (array, expr)
{
    ...
    return array.map ((el, i) => func.invoke (el, el, i, array));
}
...
function isEqual (a, b)
{
    ...
    if (a !== null
        ...
        && typeof b == "object")
    {
        var ka  = propertyNames (a);
        var kb  = propertyNames (b);
...
function query (obj, path)
{
    ...
    while (obj !== null && obj !== undefined && (path = paths.shift ()) !== undefined)
    {
        let spread = false;

        if (path == "*")
        {
            obj = isArray (obj) ? obj : (isObject (obj) ? Object.values (obj) : [obj]);
        }
        else
        if (path[0] == query.FILTER_OPEN || (spread = (path[0] == "*" && path[1] == query.FILTER_OPEN)))
        {
            ...
            {
                path = path.slice (spread ? 2 : 1, -1).trim ();
                path = path.length ? path : ".";
                path = path == "." ? "this" : path;

                let result = arrayMap ([obj], path)[0];

                if (result)
                {
                    if (spread)
                    {
                        obj = isArray (result) ? result : [result];
                    }
                }
                else
                {
    ...
    for (let c of [...path])
    {
        ...
        else
        {
            ...
            if (c == query.FILTER_CLOSE)
            {
                ...
                if (opens < 0)
                {
                    throw new Error (`Extra filter closing character in the path expression '${path}'.`);
                }
            }
