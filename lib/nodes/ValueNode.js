    ...
    async resolveSubstitutions (scope)
    {
        ...
        switch (type)
        {
            ...
            case "object":
                ...
                for (let v of values)
                {
                    for (let i in v)
                    {
                        value[i] = _.isObject (value[i]) && _.isObject (v[i]) ? Node.merge (value[i], v[i]) : v[i];
                    }
                }
        ...
        if (srac)
        {
            if (!_.isArray (sracValue))
            {
                if (sracValue === null)
                {
                    sracValue = _.query (this.root.lastScope, this.context.fullPath);
                }

                if (sracValue === null || sracValue === undefined)
                {
                    Node.assign (this.root.lastScope, this.context.fullPath, sracValue = []);
                }
                else
                {
                    this.error (`Self-referential substitutions cannot be applied to a non-array value '${JSON.stringify (sracValue)}'.`, this.$.sracToken);
                }
            }
