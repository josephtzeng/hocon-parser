...
const FILTER_PATTERN    = /^([a-z][a-z0-9]+(?:-[a-z0-9]+)*)\s*(\((.*)\))?$/;
...
Context.Url = class ContextUrl
{
    ...
    join (path)
    {
        this.$.pathname = no_url.parse (no_path.join (this.$.pathname, path)).pathname;
    }
...
Context.parseUrl  = function (resource, parentContext)
{
    ...
    if (!url.match (PROTOCOL_PATTERN))
    {
        ...
        {
            let name    = match[1];
            let options = match[3] && match[3].trim ();
    ...
    if (WINDOWS
        && u.protocol
        && !u.slashes
        && no_path.win32.resolve (decodeURIComponent (u.href)) == url.slice (0, u.protocol.length).toLowerCase () + url.slice (u.protocol.length))
    {
        root = u.protocol;
        u.protocol = null;
        u.pathname = url;
    ...
    if (!u.protocol)
    {
        ...
        if (parentSource)
        {
            u = no_url.parse (Context.parseUrl (parentContext.source.appendPath (url)).url, true);
        }
    }
